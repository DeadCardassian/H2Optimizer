...
class H2OLlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        cnt_test += 1
        if cnt_test == 350:
            torch.save(query_states, 'q.pt')
            torch.save(key_states, 'k.pt')
        ...
        token_len = 64
        n_samples = 20
        near_end = (token_len * n_samples - 2) * 32
        fa_flag = 1

        if past_key_value is not None and (self.token_cnt == token_len):  # 下一个prompt的更新清零
            self.token_cnt = 0
        if past_key_value is not None and (self.token_cnt % 8 == 0):
        ...
        if (len(time_seq_1)==near_end):
            ...
            print("AVE TOTAL ATTENTION TIME:")
            print("{:.10f}".format(ave_total))
            print("AVE ORI ATTENTION TIME: {:.10f}".format(ave_3))
