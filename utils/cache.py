...
class HHCache(Cache):
    ...
    def update_slimming(
        ...
        if self.get_seq_length(layer_idx) > self.window_length:
            ...
            _, keep_hh_index = torch.topk(seq_scores, self.num_hh_tokens, dim=-1)  #The previous sequence used to filter H2 is retained
            ...
            keep_index = torch.cat([keep_hh_index, keep_local_index], dim=-1) #combined H2 and near index
    ...
    def update_slimmingH(
        ...
        Slimming the cache based on accumulated attention scores, only keep heavy-hitters + local tokens.
        """
        # Downsample the current step's attention scores by num_kv_groups and sum them
        ...
        if len(self.accumulated_attention_scores) <= layer_idx:
            ...
            # Check if there are overlapping parts to add (if new_len == num_new_tokens, :-num_new_tokens will produce an empty slice)
        ...
        # If the cached sequence length exceeds window_length, perform slimming
        if self.get_seq_length(layer_idx) > self.window_length:
            acc_scores = self.accumulated_attention_scores[layer_idx]
            total_len = acc_scores.shape[-1]

            # Heavy-hitter score range (select from older contexts)
            hh_end = total_len - self.window_length + self.num_hh_tokens
            if hh_end > 0:  # Avoid negative/invalid indices
                seq_scores = acc_scores[:, :, :hh_end]
                # Select indices of heavy-hitters using topk
                _, keep_hh_index = torch.topk(seq_scores, self.num_hh_tokens, dim=-1, largest=True, sorted=True)
            else:
                # If hh_end <= 0, it means window_length exceeds current accumulated length, use all tokens directly
            ...
            # Local token indices (latest tokens)
            ...
            # Combine heavy-hitters and local indices
            keep_index = torch.cat([keep_hh_index, keep_local_index], dim=-1)  # [bs, num_heads, keep_len]

            # Use gather to directly index and keep the selected tokens
