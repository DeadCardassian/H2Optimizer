...
class H2OLlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.token_cnt = 0  # count tokens generated
        self.token_len = 64 # default = 64
        self.n_skip = 16  # 8 means skip 7 and update kvcache 1. token_len % n_skip = 0 is required
    ...
    def forward(
        ...
        **kwargs,
        ...
        #st4 = time.time()
        n_samples = 31
        near_end = (self.token_len * n_samples - 2) * 32  # print time statistics at near_end;  32 = num_layer
        
        h2opt_flag = 1
        fa_flag = 0

        if past_key_value is not None and (self.token_cnt >= self.token_len):  # clean token counter for next prompt
            self.token_cnt = 0
        if past_key_value is not None and (self.token_cnt % self.n_skip == 0):  # every n_skip times, do H2O
            ...
            #st2 = time.time()
            if h2opt_flag:
                past_key_value.update_slimmingH(attn_weights, self.num_key_value_groups, self.layer_idx) # H2O improved ver
            else:
            ...
            #global use_amount
            #use_amount += 1
            #if use_amount % 10000 == 0:
            #    print(use_amount)
            #st3 = time.time()
            st2 = time.time()
            if fa_flag:  # FLASH ATTENTION for test
                #FLASH ATTENTION
                dropout_p = 0.0
                softmax_scale = None
                causal = True
        ...
        try: st2
            ...
            #print("check1")
        try: st3
        except UnboundLocalError as e: st3 = time.time()
        #try: st4
        #except UnboundLocalError as e: st4 = time.time()
        ed = time.time()
        ...
        if (len(time_seq_1)==near_end):
            ...
            print("AVE TOTAL ATTENTION TIME: {:.10f}".format(ave_total))
            print("FLASH ATTENTION TIME: {:.10f}".format(ave_2 - ave_3))
            #print("KVCACHE UPDATE TIME: {:.10f}".format(ave_2 - ave_3))
            #print("AVE FLASH ATTENTION TIME: {:.10f}".format(ave_4))
